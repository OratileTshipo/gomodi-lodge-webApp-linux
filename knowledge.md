...
- **`CODE-QUALITY-review.md`** — code-quality audit + what the 2026-08-12 passes fixed (lint errors, `bathOrShower` bug, BookingForm + AdminDashboard + homepage splits, shared booking helpers, vitest, CI job). READ before refactoring — every listed follow-up is now DONE, including the env-file untrack (2026-08-15, PR #38; rotating any secrets that were ever pushed is the only owner-side remainder).
...
- **Middleware:** `main` has `middleware.ts` (CSP nonce, rate limiting, host validation) — Next 16 warns it should be renamed `proxy.ts`; harmless warning only. `.env`/`.env.local` were tracked in git (contained live DB + WhatsApp placeholders) — **untracked 2026-08-15 (PR #38), then purged from ALL history 2026-08-15 via `git filter-repo`** (every commit SHA changed; re-clone local copies; rotate any real secrets that were ever pushed). Canonical env template: `.env.example`.
