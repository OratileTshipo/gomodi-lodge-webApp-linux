...
// Lelz Business Enterprise's own commercial packages — reproduced verbatim
// from the owner-approved poster content (LELZ_PARTNERSHIP_SPEC.md §2).
type LelzPackage = {
  name: string;
  tag: string;
  inclusions: string[];
...
const PACKAGE_GRADUATION_INCLUSIONS = [
  ...
  "Balloon Garland (400 Balloons)",
  "Guest of honour chair",
  "Tiffany Gold chairs & cushions",
  "Glass Tables",
  "Centrepiece",
  "Baby accessories",
  "Artificial flowers",
    ...
    <main className="page-transition">
      ...
      <section id="packages" className="py-16 md:py-24 bg-cream-light">
        <div className="max-w-6xl mx-auto px-6">
                ...
                <div className="p-6 md:p-8">
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {pkg.inclusions.map((item) => (
                      <span
                        key={item}
                        className="bg-cream text-ink text-xs px-2.5 py-1 rounded-full border border-walnut/10"
                      >
                        {item}
                      </span>
                    ))}
                        ...
                        {pkg.prices.map((row) => (
                          <tr key={row.pax} className="border-t border-walnut/10">
                            ...
                            <td className="px-4 py-2.5 text-terracotta-dark font-semibold">
                              {row.price}
                              {row.note && (
                                <span className="text-stone text-xs font-normal"> · {row.note}</span>
                              )}
                            </td>
                          </tr>
                        ))}
          ...
          <div
            className="mt-6 bg-white rounded-2xl border-2 border-gold-dark card-shadow p-6 md:p-8 motion-scale-in motion-ready card-lift"
            data-stagger="5"
          >
            <div className="flex flex-col md:flex-row md:items-center gap-5">
              ...
              <a
                href="#inquiry-form"
                className="btn-gold px-5 py-2.5 rounded-lg font-semibold text-sm inline-flex items-center justify-center gap-2 shadow-sm shadow-gold/20 hover:shadow-md hover:shadow-gold/30 btn-press"
              >
