...
If the file is under app/book/ or app/admin/, note that the orange palette there is a known, already-logged gap rather than reporting it as a new finding — just confirm nothing NEW has drifted further from the tokens. If you need to locate files, spawn the file-picker agent. Produce a short report grouped by file and report it via set_output. Do not edit any files, and do not propose a specific fix for the orange-palette gap — that requires a decision from Oray about which pages are in scope for re-styling.`,
  stepPrompt:
    'Continue until every file in scope has been checked, then report findings with set_output. Never modify files.',
}
