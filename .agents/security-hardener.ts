...
const definition: AgentDefinition = {
  ...
  spawnableAgents: ['basher', 'code-searcher', 'code-reviewer-deepseek-flash', 'researcher-web'],
  spawnerPrompt:
    'Spawn this agent to review and harden authentication (OTP flow) and admin API authorization — the two High-severity security gaps logged in knowledge.md. Never spawn it to add new user-facing features.',
  inputSchema: {
    prompt: {
      type: 'string',
...
3. Add a server-side session check to every app/api/admin/* route, reusing the same validation logic as /api/auth/me rather than writing a second, divergent implementation. Use the code-searcher agent to find every route in app/api/admin/*.
...
5. If you need to verify behavior in a browser or run tests, use the basher agent to run commands; if you need current best-practice guidance on secure OTP/session handling, spawn the researcher-web agent.
6. Update knowledge.md: only remove or downgrade the "dev-grade" warnings in Known Issues, Guardrails, and the Architecture Decisions table once the fix is verified working end-to-end — not on your own say-so alone.
7. Spawn code-reviewer-deepseek-flash to review the diff before finishing, given the sensitivity of this change, then report your before/after summary via set_output.`,
  stepPrompt:
    'Continue hardening only auth and admin-API authorization. If closing a gap requires a new architecture decision (e.g., how OTPs get delivered for real), stop and ask rather than deciding unilaterally. Use set_output when both gaps are closed or when blocked on a decision that needs Oray.',
}
